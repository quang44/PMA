...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{translate('Danh sách đại lý')}}</h1>
        </div>
        <div class="col-md-6 text-md-right">
            <a href="{{ route('affiliate.employee.create') }}" class="btn btn-circle btn-info">
...
{{--                        <th>#</th>--}}
                        ...
                        <th data-breakpoints="lg">{{translate('Address')}} /{{translate('branch')}}</th>
                        <th data-breakpoints="lg">{{translate('depot')}}</th>
                        <th data-breakpoints="lg" class="text-center">{{translate('Status')}}</th>
...
{{--                            <td>{{ ($key+1) + ($users->currentPage() - 1)*$users->perPage() }}</td>--}}
                            ...
                            <td>@if($user->email!=null) {{$user->email}} @else  <span
                                    class="text-danger">Chưa có email</span> @endif</td>
                            <td>
                                <a href="{{ route('affiliate.kol.index', ['referred_by' => $user->id])}}">{{$user->phone}}</a>
                            </td>
                            {{--                                <td>--}}
                            {{--                                    {{ $user->referral_code }}--}}
                            {{--                                </td>--}}
                            ...
                            + {{$address->province!=null?$address->province->name:''}} - {{$address->district!=null?$address->district->name:''}}-{{$address->ward!=null?$address->ward->name:''}} <br>
                                @endforeach
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                ...
                                <td class="text-right">
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('affiliate.employee.edit', encrypt($user->id))}}" title="{{ translate('Sửa thông tin') }}">
                                        <i class="las la-edit"></i>
                                    </a>
                                    @if($user->status ==1)
                                        <a href="#" class="btn btn-soft-success btn-icon btn-circle btn-sm" onclick="confirm_lever_up('{{route('affiliate.employee.updateToAgent', encrypt($user->id))}}');" title="{{ translate('Approve') }}">
                                            <i class="las la-level-up-alt"></i>
                                        </a>
                                    @else
                                    @if($user->banned != 1)
                                        <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm" onclick="confirm_ban('{{route('customers.ban', encrypt($user->id))}}');" title="{{ translate('Lock account') }}">
                                            <i class="las la-user-slash"></i>
                                        </a>
                                    @else
                                        <a href="#" class="btn btn-soft-success btn-icon btn-circle btn-sm" onclick="confirm_unban('{{route('customers.ban', encrypt($user->id))}}');" title="{{ translate('Active account') }}">
