...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        $gifts=GiftRequest::where('status',1)->count();
        $depots=User::where('belong',0)->where('user_type','employee')->count();
        $agents=User::where('belong','>',0)->where('user_type','employee')->count();

        return view('backend.dashboard', compact('gifts','agents','depots','root_categories', 'cached_graph_data', 'total_withdraw', 'total_not_withdraw', 'total_active'));
    }
