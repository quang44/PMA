        ...
        public function signup(AuthRequest $request)
        {
            ...
            if ($user != null) {
                return response()->json([
                    'result' => false,
                    'message' => translate('Số điện thoại của bạ đã được sử dụng'),
                    'data' => null
                ], 200);
            }
...
//        TelegramService::sendMessageGomdon($text);
            return response()->json([
                'result' => true,
                'message' => translate('Đăng ký tài khoản thành công ,vui lòng xác thực tài khoản '),
                'data' => $user
            ]);
       ...
       User::where('phone',$request->phone)->update([
        ...
        public function confirmCode(Request $request)
        {
            ...
            if ($user != null && $user->verification_code == $request->verification_code) {
                ...
                return response()->json([
                    'result' => true,
                    'message' => translate('Tài khoản của bạn  đã được xác thực ,vui lòng đăng nhập'),
                ], 200);
            } else {
//            $user->delete();
                return response()->json([
                    'result' => false,
                    'message' => translate('Mã không khớp, bạn có thể yêu cầu gửi lại mã'),
                ], 200);
            }
        }
...
//    }
        ...
        public function update(Request $request)
        {
            ...
            if ($request->password_old != null && Hash::check($request->password_old, $user->password)) {
            ...
            return response()->json([
                'result' => true,
                'message' => translate('Thông tin cá nhân của bạn đã được cập nhật thành công')
            ]);
        }
