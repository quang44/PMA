...
//    function Dinh_Vi(Request $request)
//    {
//        $lat = $request->lat;
//        $lng = $request->lng;
//        $province = Province::query()->whereRaw(6371 * ACOS(COS(ceil($lat)) * COS(ceil($lat)) * COS(ceil($lng) - ceil($lng)) + SIN(ceil($lat)) * SIN(ceil($lat))) <= $radius)
//         return response(['result' => true, 'data' => $province, 'message' => 'success']);
//    }
    ...
    function getProvince(Request $request)
    {
        ...
        $provinces = DB::table('province')
            ->select('name','id')
            ->selectRaw('(6371000 * acos(cos(radians(?)) * cos(radians(SUBSTRING_INDEX(latlng, ",", 1))) * cos(radians(SUBSTRING_INDEX(latlng, ",", -1)) - radians(?)) + sin(radians(?)) * sin(radians(SUBSTRING_INDEX(latlng, ",", 1))))) AS distance', [$lat, $lng, $lat])
            ->whereRaw('(6371000 * acos(cos(radians(?)) * cos(radians(SUBSTRING_INDEX(latlng, ",", 1))) * cos(radians(SUBSTRING_INDEX(latlng, ",", -1)) - radians(?)) + sin(radians(?)) * sin(radians(SUBSTRING_INDEX(latlng, ",", 1))))) <= ?', [$lat, $lng, $lat, $radius])
            ->orderBy('distance')
            ->first();
...
//
//        DB::table('province')
//            ->select('province.name as province', 'district.name as district', 'ward.name as ward')
//            ->selectRaw('
//        (
//            6371000 * acos(
//                cos(radians(SUBSTRING_INDEX(province.latlng, ",", 1))) *
//                cos(radians(SUBSTRING_INDEX(province.latlng, ",", -1))) *
//                cos(radians(SUBSTRING_INDEX("' . $latlng . '", ",", 1)) - radians(SUBSTRING_INDEX(province.latlng, ",", 1))) +
//                sin(radians(SUBSTRING_INDEX(province.latlng, ",", 1))) *
//                sin(radians(SUBSTRING_INDEX("' . $latlng . '", ",", 1)))
//            )
//        ) AS distance')
//            ->join('district', 'district.province_id', '=', 'province.id')
//            ->join('ward', 'ward.district_id', '=', 'district.id')
//            ->whereRaw('(6371000 * acos(
//                    cos(radians(SUBSTRING_INDEX(province.latlng, ",", 1))) *
//                    cos(radians(SUBSTRING_INDEX(province.latlng, ",", -1))) *
//                    cos(radians(SUBSTRING_INDEX("' . $latlng . '", ",", 1)) - radians(SUBSTRING_INDEX(province.latlng, ",", 1))) +
//                    sin(radians(SUBSTRING_INDEX(province.latlng, ",", 1))) *
...
//        dd($province);
        ...
        return response(['data' => $data, 'result' => true, 'message' => 'success']);
    }
